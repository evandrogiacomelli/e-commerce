...
public class OrderControllerTest {
    ...
    //     when(orderService.createOrder(any(UUID.class))).thenThrow(new InvalidOrderDataException("Customer ID cannot be null"));
    //     // Act & Assert
    //     mockMvc.perform(post("/orders")
    //             .contentType(MediaType.APPLICATION_JSON)
    //             .content(objectMapper.writeValueAsString(request)))
    //             .andExpect(status().isBadRequest());
    //     verify(orderService, times(1)).createOrder(any(UUID.class));
    // }
    @Test
    ...
    void shouldGetOrdersByCustomerId() throws Exception {
        ...
        mockMvc.perform(get("/orders/customer/{customerId}", testCustomer.getId())
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(2))
                .andExpect(jsonPath("$[0].customerId").value(testCustomer.getId().toString()));
    ...
    void shouldReturnBadRequestWhenAddingItemWithInvalidData() throws Exception {
        ...
        when(orderService.addItemToOrder(eq(testOrder.getId()), any(UUID.class), anyInt(), any(BigDecimal.class)))
        ...
        verify(orderService, times(1)).addItemToOrder(eq(testOrder.getId()), any(UUID.class), anyInt(), any(BigDecimal.class));
    }
    ...
    void shouldReturnBadRequestWhenRemovingInvalidItem() throws Exception {
        // Arrange
        when(orderService.removeItemFromOrder(eq(testOrder.getId()), eq(UUID.randomUUID())))
                .thenThrow(new IllegalArgumentException("Product not found in order"));

        // Act & Assert
        mockMvc.perform(delete("/orders/{orderId}/items/{productId}", testOrder.getId(), UUID.randomUUID())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest());

        verify(orderService, times(1)).removeItemFromOrder(eq(testOrder.getId()), any(UUID.class));
    }
    ...
    void shouldReturnBadRequestWhenUpdatingItemQuantityWithInvalidData() throws Exception {
        // Arrange
        OrderItemRequest request = new OrderItemRequest(testProduct.getId(), -1, new BigDecimal("10.00"));
        when(orderService.updateItemQuantityInOrder(eq(testOrder.getId()), any(UUID.class), anyInt()))
                .thenThrow(new IllegalArgumentException("Quantity must be greater than zero."));
        ...
        verify(orderService, times(1)).updateItemQuantityInOrder(eq(testOrder.getId()), any(UUID.class), anyInt());
    }
